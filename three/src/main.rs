...
fn set_of_coords(directions: &str) -> HashSet<(i32, i32)> {
    let mut coords: HashSet<(i32, i32)> = HashSet::new();
    let mut location = (0, 0);
    for direction in directions.split(&",") {
        let (direction, number) = direction.split_at(1);
        let number = number.parse::<i32>().unwrap();
        for _ in 0..number {
            location = next_location(location, direction);
            coords.insert(location);
        }
    }
    coords
}

fn next_location(location: (i32, i32), direction: &str) -> (i32, i32) {
    match direction {
        "D" => (location.0, location.1 - 1),
        "L" => (location.0 - 1, location.1),
        "R" => (location.0 + 1, location.1),
        "U" => (location.0, location.1 + 1),
        _ => panic!("Unrecognized string"),
    }
}

fn closest_intersection(directions1: &str, directions2: &str) -> (i32, i32) {
    set_of_coords(directions1)
        .intersection(&set_of_coords(directions2))
        .min_by(|x, y| {
            (length_to(directions1, **x) + length_to(directions2, **x))
                .cmp(&(length_to(directions1, **y) + length_to(directions2, **y)))
        })
        .unwrap()
        .to_owned()
}

fn length_to(directions: &str, location: (i32, i32)) -> i32 {
    let mut length = 0;
    ...
    panic!("Location is not crossed by wire")
}

fn length_to_intersection(directions1: &str, directions2: &str) -> i32 {
    let closest_intersection = closest_intersection(directions1, directions2);
    length_to(directions1, closest_intersection) + length_to(directions2, closest_intersection)
}
...
fn main() {
    ...
    println!("{:?}", length_to_intersection(wire1, wire2));
}
...
mod tests {
    ...
    fn test_length_to() {
        ...
        assert_eq!(length_to_intersection(wire1, wire2), 610);
    }
    ...
    fn test_length_to_2() {
        ...
        assert_eq!(length_to_intersection(wire1, wire2), 410);
    }
}
